...
function VariationClasses ({ data }) {
  ...
  useEffect(() => {   
    ...
    for (let i = 1; i < M + 1; i++) {
      const lowerBound = (Xmin + (i - 1) * h);
      const upperBound = (lowerBound + h);
    ...
    function computeFrequency(lowerBound, upperBound){
        ...
        data.forEach((number, index) => {
          if(index !== data.length - 1) {
            if((number > lowerBound || Math.abs(number - lowerBound) < eps) && number < upperBound) {
              n++
            }
          }
          else {
            if((number > lowerBound || Math.abs(number - lowerBound) < eps) && (number < upperBound || Math.abs(number - upperBound) < eps)) {
              n++
            }
          }
    ...
    const ecdf = ceil => {
      const eps = Math.pow(10, -5);
      return data.filter(el => el < ceil || Math.abs(ceil - el) < eps).length / data.length;
    }
    ...
    setClassesList( //set - async f 
        classBoundaries.map((wide, index) => {
            return {
              ...
              relativeFrequency: computeFrequency(wide.lowerBound, wide.upperBound) / data.length,
              ECDF: ecdf(wide.upperBound)
            }
        })
    );
